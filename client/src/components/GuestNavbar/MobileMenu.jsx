...
const MobileMenu = ({
  ...
  useEffect(() => {
    const update = () => setCurrentTime(formatCurrentTime());
    update();

    const interval = typeof window !== 'undefined' ? window.setInterval(update, 30000) : null;
          ...
          >
            ...
            <div className="flex h-full flex-col px-6 pb-10 pt-20 text-slate-900 dark:text-white">
              ...
              >
                ...
                <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-xs font-medium uppercase tracking-[0.28em] text-slate-600 dark:border-white/20 dark:bg-white/10 dark:text-white/70">
              ...
              >
                ...
                <button
                  type="button"
                  onClick={onSignOut}
                  className="text-sm font-semibold uppercase tracking-[0.24em] text-red-500 transition hover:text-red-600 dark:text-red-300 dark:hover:text-red-200"
                >
                  Sign Out
                </button>
              </Motion.nav>
